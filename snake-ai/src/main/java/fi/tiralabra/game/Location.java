...
/**
 * This class is sort of a relic from the pre refractor GameArea. Provides way
 * to keep (x,y) together and calculate distances.
 *
 * @author vili
 */
public class Location {
    ...
    public int distance(Location loc) {
        ...
        int throughRoof = this.straightDistance(new Location(this.x, 1, this.area))
                + loc.straightDistance(new Location(this.x, this.area.getHeight() - 2, this.area)) + 1;
        int throughFloor = this.straightDistance(new Location(this.x, this.area.getHeight() - 2, this.area))
                + loc.straightDistance(new Location(this.x, 1, this.area)) + 1;
        int throughLeft = this.straightDistance(new Location(1, this.y, this.area))
                + loc.straightDistance(new Location(this.x, this.area.getWidth() - 2, this.area)) + 1;
        int throughRight = this.straightDistance(new Location(this.area.getWidth() - 2, this.y, this.area))
                + loc.straightDistance(new Location(1, this.y, this.area)) + 1;
